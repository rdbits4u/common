...
const NUMBER_OF_SPACES_IN_LINE: usize = 16;
const NUMBER_OF_BYTES_BEFORE_SPACE: usize = 1;
...
/// Section Seperator can be changed but must not be more than two bytes
/// in length.
...
/// Bytes + Spaces between bytes + space before the bytes (the space after is
/// counter in NUMBER_OF_SPACES_IN_LINE)
/// 16 * 2 + 8 + 1 = 32 + 8 + 1 = 41
const HEX_CONTENTS_SECTION_LENGTH = BYTES_IN_LINE * BYTE_PRINTING_LENGTH +
        NUMBER_OF_SPACES_IN_LINE + 1;
...
const TOTAL_LENGTH = ADDRESS_SECTION_LENGTH + HEX_CONTENTS_SECTION_LENGTH +
        TEXT_CONTENTS_SECTION_LENGTH;
...
fn printHexDumpHeader(offset: usize) !void
{
    ...
    const fmt = std.fmt.comptimePrint("0x{{x:0>{}}}: ",
            .{ADDRESS_PRINTING_LENGTH});
...
fn printHexDumpBody(contents: []const u8) !void
{
    ...
    {
        const non_printable = !std.ascii.isPrint(byte);
        if (non_printable)
        {
            if (byte == NULL_BYTE)
            {
        ...
        const fmt = std.fmt.comptimePrint("{{x:0>{}}}",
                .{BYTE_PRINTING_LENGTH});
        try writer.print(fmt, .{byte});
        try printColor(resetColor);
        if (i % NUMBER_OF_BYTES_BEFORE_SPACE ==
                (NUMBER_OF_BYTES_BEFORE_SPACE - 1))
        {
            _ = try writer.write(" ");
        }
    }
...
fn printHexDumpTrailer(contents: []const u8) !void
{
    ...
    if (contents.len < BYTES_IN_LINE)
    {
        const padding_length = (BYTES_IN_LINE - contents.len) * 
                BYTE_PRINTING_LENGTH + (NUMBER_OF_SPACES_IN_LINE -
                (contents.len / NUMBER_OF_BYTES_BEFORE_SPACE));
...
pub fn printFilePath(path: []const u8) !void
{
    ...
    const formatted = std.fmt.comptimePrint("{s} File Path: ",
            .{sectionSeperator});
    _ = try writer.write(formatted);
    try printColor(filePathColor);
    const fmt = std.fmt.comptimePrint("{{s: <{}}}",
            .{TOTAL_LENGTH - formatted.len - 1 - sectionSeperator.len});
...
pub fn printTableHeader() !void
{
    ...
    const fmt = std.fmt.comptimePrint(
            "{0s}{{s: <{1}}}{0s}{{s: ^{2}}}{0s}{{s: >{3}}}{0s}\n",
            .{ sectionSeperator, ADDRESS_SECTION_LENGTH - 2 *
            sectionSeperator.len, HEX_CONTENTS_SECTION_LENGTH,
            TEXT_CONTENTS_SECTION_LENGTH - 2 * sectionSeperator.len });
...
//*****************************************************************************
/// Prints a hexdump of the provided contents, user can specify the offset
///  in the file.
